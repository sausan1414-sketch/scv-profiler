"""
Typer CLI for CSV Profiler (matches dummy repo style)
"""
...
def profile(
    ...
    format: str = typer.Option("both", "--format", help="Output format: json, markdown, both", case_sensitive=False),
):
    """
    ...
    try:
        typer.echo("\nSummary:")
        typer.echo(f" Rows: {data.get('rows')}")
        typer.echo(f" Columns: {data.get('columns')}")
        for c in data.get("column_profiles", []):
            name = c.get("name")
            typ = c.get("type")
...
if __name__ == "__main__":
    # Important: run the Typer app (keeps subcommand `profile`)
    app()
